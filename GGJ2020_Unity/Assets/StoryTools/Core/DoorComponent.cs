...
public class DoorComponent : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            OnDoorStateChanged(StoryState.Instance.DoorAState);
            StoryState.Instance.OnDoorAChanged += OnDoorStateChanged;
        }
        else
        {
            OnDoorStateChanged(StoryState.Instance.DoorBState);
    ...
    private void OnDoorStateChanged(E_DoorState newDoorState)
    {
        switch (newDoorState)
        {
            ...
            case E_DoorState.ShutHard:
                transform.DOLocalMove(_originalLocation, 0.4f, false).SetEase(Ease.OutBack);
